# Code taken from: https://github.com/DKarandikar/Flocking
# License: https://github.com/DKarandikar/Flocking/blob/master/LICENSE
# Date Accessed: 04/10/2023
# Changelog:
# Formatted to Python3
# Changed initial line 474 and 475 to strip out width and height from the kwargs; which also affected initial  479
# World init is changed to receive width and height
# changed window dimension to 800 by 800
# Added some globals
# Changed circle size
# Added checkbox use obstacles
# Added slider to control speed
# Added a function to generate random colors every 5 seconds #
...
class World:
    ...
    def setup_obstacles(self, obs_type):
        ...
        if obs_type == "line":  # Sets two vertical lines on the edges
            ...
            for i in range(45):
                k = Obstacle()
                k_x = self.width / 2 + OBS_CIRCLE_SIZE * math.cos((i + 1) * 2 * math.pi / 45)
                k_y = self.height / 2 + OBS_CIRCLE_SIZE * math.sin((i + 1) * 2 * math.pi / 45)
                k.setxy(k_x, k_y)
                self.obstacles.append(k)
...
def setColors():
    global color_hex1, color_hex2, color_hex3
    color_hex1 = random_hex_color()
    color_hex2 = random_hex_color()
    color_hex3 = random_hex_color()


def setInterval(func, time):
    """
    Call a function `random_hex_color` where i generate random colors every `x-time` seconds.
    """
    timer = threading.Timer(time, setInterval, [func, time])
    timer.start()
    func()


# calling the setcolor function every 5 seconds
setInterval(setColors, 5)
...
class Flocking(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.useobs = tk.BooleanVar()
        self.useobs.set(True)
    ...
    def create_buttons(self):
        ...
        self.obs_button = tk.Checkbutton(self.buttons, text="Use Obstacles", variable=self.useobs, width=18,
                                         onvalue=True, offvalue=False)
        self.obs_button.pack(side="top", pady=(10, 10))
        self.repulse_slider = tk.Scale(self.buttons, from_=0.1, to=10, orient="horizontal")
        self.repulse_slider.pack(side="top", pady=(10, 10))
        self.repulse_slider.set(BOID_SPEED)
    ...
    def update(self):
        ...
        global BOID_SPEED
        BOID_SPEED = self.repulse_slider.get()
    ...
    def draw_boids(self):
        ...
        for boid in self.world.boids:
            tip_x = boid.pos_x + (10 * boid.vel_x)
            tip_y = boid.pos_y + (10 * boid.vel_y)
            ...
            self.canvas.create_polygon((tip_x, tip_y,
                                        int(boid.pos_x - (3 * boid.vel_y)),
                                        int(boid.pos_y + (3 * boid.vel_x)),
                                        int(boid.pos_x + (3 * boid.vel_y)),
                                        int(boid.pos_y - (3 * boid.vel_x))),
    ...
    def draw_obstacles(self):
        ...
        if self.useobs.get():
            ...
            for obs in self.world.obstacles:
                self.canvas.create_oval(obs.oval, fill="", outline="", tags="obstacle")
...
if __name__ == "__main__":
    APP = Flocking(width=800, height=800)
    APP.mainloop()
    APP.quit()
